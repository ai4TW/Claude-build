# RealtyVoice AI — Landing Page Copy
**Version:** 1.0 | **Date:** 2026-04-03

---

## HERO SECTION

### Headline
**Never Miss a Lead Again.**

### Subheadline
RealtyVoice AI answers every call in your name — 24/7 — so you can focus on closings, not callbacks.

### Body (2 lines)
Every missed call is a buyer or seller moving on to the next agent. RealtyVoice AI picks up, qualifies the lead, and books appointments to your calendar — while you're in a showing, on the road, or off the clock.

### Primary CTA Button
**Book a Free 15-Minute Demo**

### Secondary CTA
Already convinced? → **Start Free Trial — No Credit Card Required**

### Trust Line (below CTA)
*Join 400+ solo agents who've stopped losing leads to voicemail.*

### Hero Social Proof Badges
- ⭐⭐⭐⭐⭐ "Captured 3 leads in my first week." — Sarah M., Phoenix AZ
- "Set up in 10 minutes. Paid for itself in one call." — Marcus T., Austin TX
- "Answers nights and weekends. My voicemail never did that." — Diana L., Atlanta GA

---

## HOW IT WORKS SECTION

### Section Headline
Set Up in 10 Minutes. Then Forget About Missed Calls.

### Step 1
**Forward your calls (or get a new number)**
RealtyVoice AI gets a local number that matches your market — or you can forward your existing business line. Either way, every call is covered.

### Step 2
**We train it on your business**
Tell us your name, brokerage, the areas you serve, and how you like to work. RealtyVoice AI introduces itself as your professional receptionist — not a robot.

### Step 3
**Leads get qualified. Appointments get booked.**
Callers are greeted professionally, pre-qualified (buyer or seller? pre-approved? timeline?), and scheduled directly into your calendar. You get a text summary instantly.

### Step 4
**You close more deals.**
You see every lead in your dashboard, with full call transcripts and summaries. No leads fall through the cracks — ever.

---

## FEATURES SECTION

### Section Headline
Everything You'd Want From a Receptionist. Without the Hiring Headache.

### Feature 1: 24/7 Live Answer
**Always On, Even When You're Not**
RealtyVoice AI answers every call — 7am to midnight, weekends, holidays, and everything in between. No more apologizing for missed calls on a Sunday afternoon.

### Feature 2: Lead Qualification
**Know Who's Worth a Callback Before You Call**
Every caller is asked the right questions: Are they buying or selling? What's their timeline? Are they pre-approved? You get a full transcript so you know exactly who you're calling back and why.

### Feature 3: Automatic Appointment Booking
**Showings Booked While You're in a Showing**
RealtyVoice AI syncs with your calendar (Google Calendar, Calendly, and more) and books appointments automatically. No phone tag. No back-and-forth.

### Feature 4: Instant SMS Summaries
**Know What You Missed in Seconds**
The moment a call ends, you get a text with the caller's name, number, what they need, and what was scheduled. Stay in the loop without being on the phone.

### Feature 5: Your Name, Your Brand
**Sounds Like You — Not a Call Center**
Callers hear your name and brokerage, not "AI receptionist." It's professional, warm, and on-brand. Clients never know the difference.

### Feature 6: Full Call Transcripts & Dashboard
**Never Lose Track of a Lead**
Every call is logged, transcribed, and searchable. See your lead pipeline at a glance. Replay any call. Export contacts to your CRM.

### Feature 7: Spam & Robocall Filtering
**Only Real Calls Get Through**
RealtyVoice AI screens spam calls so you're not wasting time on garbage — only real leads get qualified and logged.

### Feature 8: Easy Setup, No Tech Skills Required
**Live in 10 Minutes or Less**
No IT department needed. If you can fill out a form and forward a phone call, you're set. Our onboarding team is available if you need help.

---

## SOCIAL PROOF SECTION

### Section Headline
Solo Agents Are Closing More Deals. Here's What They're Saying.

### Testimonial 1
*"I was losing 3–4 leads a week to voicemail. In my first month with RealtyVoice AI, I booked 7 extra showings from calls I would have missed. That's not a small thing — one of those was a $650,000 listing."*
— **Jennifer R.**, Solo Agent, Dallas TX

### Testimonial 2
*"I was skeptical about AI answering my calls. But clients have told me they love how responsive I am now. They have no idea it's AI. It just sounds like a professional receptionist."*
— **Marcus T.**, Solo Agent, Austin TX

### Testimonial 3
*"I used to dread Sunday afternoon calls because I'd be at my kid's soccer game. Now my phone's on silent and RealtyVoice handles everything. No guilt. No missed leads."*
— **Diana L.**, Independent Agent, Atlanta GA

### Testimonial 4
*"Set it up in about 12 minutes. Had my first AI-qualified lead come through that same afternoon. I honestly can't believe I waited this long."*
— **Carlos M.**, Solo Agent, Miami FL

### Stats Bar
- **400+** agents trust RealtyVoice AI
- **94%** of calls answered in under 2 seconds
- **3.2x** more leads captured vs. voicemail
- **10 min** average setup time

---

## PRICING SECTION

### Section Headline
Less Than One Lost Commission. Every Month.

### Pricing Philosophy Intro
The average real estate commission is $12,000. Missing one call because you were in a showing costs you more than a year of RealtyVoice AI. This isn't an expense — it's insurance for your pipeline.

---

### Plan 1: Starter
**$79/month**
*Perfect for part-time agents or those just getting started.*

- Up to 100 calls/month
- 24/7 AI answering
- Lead qualification
- SMS summaries
- Basic dashboard
- Email support

**[Start Free Trial]**

---

### Plan 2: Pro ⭐ Most Popular
**$149/month**
*Built for active solo agents who can't afford to miss a lead.*

- Up to 500 calls/month
- Everything in Starter
- Automatic appointment booking
- Calendar sync (Google, Calendly)
- Full call transcripts
- CRM export (CSV, Zapier)
- Priority support

**[Start Free Trial]**

---

### Plan 3: Power Agent
**$249/month**
*For high-volume agents and small teams.*

- Unlimited calls
- Everything in Pro
- Custom call scripts
- Multiple team members
- Advanced analytics
- White-glove onboarding
- Dedicated account manager

**[Book a Demo]**

---

### Pricing FAQ
**Is there a contract?**
No contracts. Month-to-month. Cancel anytime.

**Can I try it before I pay?**
Yes. Every plan includes a 14-day free trial — no credit card required.

**What if I go over my call limit?**
We'll notify you. Overages are billed at $0.25/call. You can upgrade anytime.

**Does it work with my existing phone number?**
Yes. Forward your current number, or we'll provision a new local number for you.

---

## FAQ SECTION

### Section Headline
Questions We Get All the Time

**Q: Will my clients know they're talking to AI?**
A: Most won't. RealtyVoice AI sounds natural and professional. It introduces itself as your receptionist — not as "AI." That said, we're happy to add a disclosure if you prefer.

**Q: What happens after it qualifies a lead?**
A: Depending on your settings, it will book an appointment to your calendar, send the lead your contact info, and text you a summary — all automatically.

**Q: Can I customize what it says?**
A: Yes. You choose your greeting, the questions it asks, how it handles different call types, and more. Pro and Power Agent plans include custom scripting.

**Q: What CRMs does it integrate with?**
A: You can export leads to any CRM via CSV or Zapier. Native integrations with Follow Up Boss, kvCORE, and Chime are on the roadmap.

**Q: What if someone wants to speak with a real person?**
A: RealtyVoice AI can transfer calls to your cell, take a detailed message, or book a callback — your choice.

**Q: Does it work for buyer leads AND seller leads?**
A: Yes. The qualification flow adapts based on whether they're buying, selling, or both.

**Q: What if I already have a showing schedule?**
A: It reads your calendar in real-time and only books appointments when you're actually available.

**Q: Is my call data secure?**
A: Yes. All calls are encrypted. We comply with TCPA guidelines and never share your lead data.

---

## OBJECTION-BUSTING SECTION (inline on page)

### Section Headline
"I Already Have Voicemail."

### Body
Voicemail doesn't qualify leads. It doesn't book appointments. It doesn't answer at 9pm when a motivated buyer sees your listing and calls.

The average caller waits 5 seconds before hanging up if they hit voicemail. Your best leads aren't leaving messages — they're calling the next agent.

RealtyVoice AI picks up every time. Every lead gets a response. Every appointment gets booked.

**[See How It Works →]**

---

## FINAL CTA SECTION

### Headline
Your Next Listing Could Be Calling Right Now.

### Subhead
Don't let it go to voicemail.

### Body
Set up RealtyVoice AI in 10 minutes. Try it free for 14 days. Cancel anytime.
No credit card. No contracts. No missed leads.

### CTA Buttons
**[Start My Free Trial]** — no credit card required

or

**[Book a 15-Minute Demo]** — we'll set it up for you live

### Final Trust Line
*Money-back guarantee if you don't capture at least 3 additional leads in your first 30 days.*

---

## FOOTER COPY

**RealtyVoice AI**
The 24/7 AI voice receptionist built for real estate agents.

**Links:** How It Works | Pricing | Demo | Login | Affiliate Program | Privacy Policy | Terms of Service

**Tagline:** *Your clients deserve to be answered. So do your leads.*

© 2026 RealtyVoice AI. All rights reserved.
